class User < ApplicationRecord
  ...
  format:
  {
    with: %r{\A(?:(A[KLRZ]|C[AOT]|D[CE]|FL|GA|HI|I[ADLN]|K[SY]|LA|M[ADEINOST]|N[CDEHJMVY]|O[HKR]|P[AR]|RI|S[CD]|T[NX]|UT|V[AIT]|W[AIVY]))\z}, message: 'must be formatted as state initials. e.g. CA for California',
    :on => :create
  }
  ...
  def self.search(query)
    ...
    where(
      "name ILIKE :name OR username ILIKE :username",
      name: wildcarded_query,
      username: wildcarded_query
    )
  end
